...
public class LogosSimpleLog implements Runnable {
 ...
 */
    ...
    private LogosSimpleLogView  logPane = null;
    ...
    private LogosTypes.UDTLogType logType = LogosTypes.UDTLogType.UNKNOWN;
    
    private BufferedReader reader = null; // it is out at this level so the log can be "prebuffered"
    public LogosSimpleLog( String logLoc, String logName ) {
    ...
    public LogosSimpleLog( String logLoc, String logName, boolean readOnly ) {
        logStack.setSize( maxLogSize );
        this.readOnly = true;
        this.logLoc = new StringBuffer( logLoc );
        this.logName = logName;
    ...
    public LogosSimpleLog( Object caller, String logLoc, String logName ) {
        this.caller = caller;
        this.logLoc = new StringBuffer( logLoc );
        this.logName = logName;
    ...
    public static String getLevelStr( int logStateInt ) {
        if( logStateInt == LogosSimpleLog.DEBUG )
            return logLevels[ LogosSimpleLog.DEBUG ];
        else if ( logStateInt == LogosSimpleLog.FATAL )
            return logLevels[ LogosSimpleLog.FATAL ];
        else if ( logStateInt == LogosSimpleLog.GUI_TRACE)
            return logLevels[ LogosSimpleLog.GUI_TRACE ];
        else if ( logStateInt == LogosSimpleLog.ISO )
            return logLevels[ LogosSimpleLog.ISO ];
        else if ( logStateInt == LogosSimpleLog.LOG_OFF )
            return logLevels[ LogosSimpleLog.LOG_OFF ];
        else if ( logStateInt == LogosSimpleLog.MESSAGE )
            return logLevels[ LogosSimpleLog.MESSAGE ];
        else if ( logStateInt == LogosSimpleLog.TRACE )
            return logLevels[ LogosSimpleLog.TRACE ];
        else if ( logStateInt == LogosSimpleLog.WARNING )
            return logLevels[ LogosSimpleLog.WARNING ];
        else if ( logStateInt == LogosSimpleLog.CRITICAL )
            return logLevels[ LogosSimpleLog.CRITICAL ];
        else if ( logStateInt == LogosSimpleLog.ERROR )
            return logLevels[ LogosSimpleLog.ERROR ];
    ...
    public static Icon getLevelIcon( int logStateInt ) {
        return null;
//        if( logStateInt == LogosSimpleLog.DEBUG )
//            return SCPMgr.getMgr().getIcon( LogosTypes.LogGlyphs.DEBUG );
//        else if ( logStateInt == LogosSimpleLog.FATAL )
//            return SCPMgr.getMgr().getIcon( LogosTypes.LogGlyphs.FATAL );
//        else if ( logStateInt == LogosSimpleLog.GUI_TRACE)
//            return SCPMgr.getMgr().getIcon( LogosTypes.LogGlyphs.TRACE );
//        else if ( logStateInt == LogosSimpleLog.ISO )
//            return SCPMgr.getMgr().getIcon( LogosTypes.LogGlyphs.SPECIAL );
//        else if ( logStateInt == LogosSimpleLog.LOG_OFF )
//            return SCPMgr.getMgr().getIcon( LogosTypes.LogGlyphs.LOG_INFO );
//        else if ( logStateInt == LogosSimpleLog.MESSAGE )
//            return SCPMgr.getMgr().getIcon( LogosTypes.LogGlyphs.MESSAGE );
//        else if ( logStateInt == LogosSimpleLog.TRACE )
//            return SCPMgr.getMgr().getIcon( LogosTypes.LogGlyphs.TRACE );
//        else if ( logStateInt == LogosSimpleLog.WARNING )
//            return SCPMgr.getMgr().getIcon( LogosTypes.LogGlyphs.WARNING );
//        else if ( logStateInt == LogosSimpleLog.CRITICAL )
//            return SCPMgr.getMgr().getIcon( LogosTypes.LogGlyphs.FATAL );
//        else if ( logStateInt == LogosSimpleLog.ERROR )
//            return SCPMgr.getMgr().getIcon( LogosTypes.LogGlyphs.FATAL );
            ...
            try {
                ...
                if( logRead != null ) {
                    ...
                    while( it.hasNext() ) {
                        String t = it.next();
                        if( t != null ) {
                            logPane.addEntry( new LogosSimpleLogEntry( t ) );
        ...
        while( readOnly ) {
            ...
            try {
                ...
                while( reader.ready()  ) {
                    ...
                    if( logPane != null ) {
                        logPane.addEntry( new LogosSimpleLogEntry( reader.readLine() ) );
                        logLinesRead++;
...
//                SCPMgr.getMgrLog().write( "Out of Memory in GesLog: " + e.getMessage());
    ...
    synchronized public void write( int level, String message ) {
        ...
        if( level >= logLevels.length ) {
            ...
            if( logPanel != null ) {
                ...
                logPane.addEntry( new LogosSimpleLogEntry( msg + "\n" ) ) ;
                logPane.invalidate();
            }
        }
    }
    ...
    public void setLogPanel( JPanel panel ) {
        ...
        if( logPanel != null ) {
            ...
            logPane.addKeyListener(new java.awt.event.KeyAdapter() {
                public void keyPressed(java.awt.event.KeyEvent evt) {
                    if( evt.getKeyCode() == java.awt.event.KeyEvent.VK_ENTER ) {
                        logPane.addEntry( new LogosSimpleLogEntry("      \n" ) );
                        // evt.consume();
                    } else {
                        evt.consume();
                    }
    ...
    public void setLogLevel( String newLevel, int watchBits, int ignoreBits ) {
        ...
        if( logType == LogosTypes.UDTLogType.PARALLAXMGR ) {
            logLevel = overAllLevel;
            write( logLevel, "GesLog::setLogLevel: log level is now: " + logLevels[ logLevel ]);
        } else if( logType == LogosTypes.UDTLogType.DSS ) {
            // ParallaxMgr.getMgr()
        }
...
//        SCPMgr.getMgr().setLogLinesBuffer(newSize);
    ...
    public LogosTypes.UDTLogType getLogType() {
        return logType;
    }
    
    public void setLogType(LogosTypes.UDTLogType logType) {
